...
void main() {
  ...
  if(intChannel[1] == 0) {
    ...
    for(int i = 0; i < 32; ++i) {
      ...
      color += texture(texChannel[0], tc + vec2(offset * floatChannel[0] / texSize.x, 0.0f)).rg * gaussian(offset, deviation);
    }
  } else {
    // vertical blur
    for(int i = 0; i < 32; ++i) {
      if(i > intChannel[0]) { break; }
      float offset = (float(i) - halfBlur) / float(intChannel[0]);
      color += texture(texChannel[0], tc + vec2(0.0f, offset * floatChannel[0] / texSize.y)).rg * gaussian(offset, deviation);
    }
  }
  fragcolor = color;
}